...
logging.info(f'Your chosen parameters to PLOT: predval_start_date: "{predval_start_date}", predval_end_date: "{predval_end_date}", pollutants: {pollutants}, trainset: {trainset}, date_of_input: "{date_of_input}"')
...
predictions_path = f'/ML_Output/HOURLY_OUTPUTS/{pollutant}/{predval_start_date}_{predval_end_date}/2023-05-22/hour_0_maps_HOURLY_TEST'                                   # pollutant, predval_start_year, predval_end_year, date_of_input
#data_handler.config.select_ml_paths(path_to_return='output_parquet_path_struct').format(pollutant, predval_start_year, predval_end_year, date_of_input)
...
pollutant_prediction_data = data_handler.parquet_reader(f'/ML_Input/HOURLY_DATA/data-{pollutant}_{predval_start_date}_{predval_end_date}.parquet', features=['*'])#.dropna(subset=[pollutant]).drop_duplicates([pollutant, 'Gridnum1km', 'date', 'hour'])    # MODIFICAR PATH PARA INPUT DATA DEL PREDICTION
...
a = pollutant_prediction_data.withColumn('weekday', dayofweek('date'))
display(a)


# COMMAND ----------

a.toPandas().weekday.value_counts()

# COMMAND ----------

predictions_data.toPandas().hour.value_counts()
