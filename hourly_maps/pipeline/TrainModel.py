...
# In case we have different target variables i.e.: eRep and e1b.
  for target in trainset:
    ...
    X_train, X_test, Y_train, Y_test = ml_worker.split_data(df=pollutant_train_data, train_size=0.7, label=trainset)
    ...
    duplicated_rows = data_handler.find_duplicates(df1=spark.createDataFrame(X_train), df2=spark.createDataFrame(X_test), cols_to_compare=['GridNum1km','date'])
    ...
    X_train = X_train[[col for col in X_train.columns if col not in ['GridNum1km', 'AreaHa', 'datetime_begin', 'datetime_end', 'resulttime', 'eucode', 'ns', 'polu', 'sta', 'Lat', 'Lon', 'AirQualityStationEoICode', 'date']]]                                        
    X_test = X_test[[col for col in X_test.columns if col not in ['GridNum1km', 'AreaHa', 'datetime_begin', 'datetime_end', 'resulttime', 'eucode', 'ns', 'polu', 'sta', 'Lat', 'Lon', 'AirQualityStationEoICode', 'date']]]   
...
#shall we include as a feature the pollutant's concetration during the previous hour?
