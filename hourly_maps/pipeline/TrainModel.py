...
# In case we have different target variables i.e.: eRep and e1b.
  for target in trainset:
    ...
    pollutant_train_data = data_handler.parquet_reader(f'/ML_Input/HOURLY_DATA/episodes/data-{pollutant}_{train_start_year}_{train_end_year}-all_episodes.parquet', features=selected_cols_pollutants).dropna(subset=[pollutant]).drop_duplicates([pollutant, 'Gridnum1km', 'date', 'hour'])
    ...
    pollutant_validation_data = data_handler.parquet_reader(f'/ML_Input/HOURLY_DATA/episodes/data-{pollutant}_{predval_start_year}_{predval_end_year}-all_episodes.parquet', features=selected_cols_pollutants).dropna(subset=[pollutant]).drop_duplicates([pollutant, 'Gridnum1km', 'date', 'hour'])
    ...
    df_validation = pollutant_validation_data.drop('GridNum1km','AreaHa', 'level3_code', 'adm_country', 'date', 'datetime_end', 'datetime_begin').toPandas()    
