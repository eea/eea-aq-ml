...
# In case we have different target variables i.e.: eRep and e1b.
  for target in trainset:
    ...
    pollutant_train_data = pollutant_train_data.sample(0.2)
    ...
    df_train = pollutant_train_data.drop('GridNum1km','AreaHa', 'level3_code', 'adm_country', 'date', 'datetime_end', 'datetime_begin').toPandas()   
...
# display(pollutant_validation_data)
# print(pollutant_validation_data.count())
...
try:
  ...
  new_params.to_csv('/dbfs'+data_handler.file_system_path + f'/ML_Input/episodes/hyper_params-{pollutant}_trainEnd{train_end_year}_traistart{train_start_year}_withCAMS.csv', index=False) 

except FileNotFoundError:
  scores_df.to_csv('/dbfs'+data_handler.file_system_path + f'/ML_Input/episodes/hyper_params-{pollutant}_trainEnd{train_end_year}_traistart{train_start_year}_withCAMS.csv', index=False) 
  existing_params = scores_df
...
# pollutant='PM25'
data_handler = DataHandler(pollutant)
existing_params = pd.read_csv('/dbfs'+data_handler.file_system_path + f'/ML_Input/episodes/hyper_params-{pollutant}_trainEnd{train_end_year}_traistart{train_start_year}_withCAMS.csv').drop_duplicates()
display(existing_params)
