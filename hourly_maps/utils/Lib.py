...
"""
================================================================================
Notebook containing core functions commonly used in different notebooks such as data collectors/manipulation. These classes are configured by using the config.py file. 
We should not need to modify nothing except if we are willing to add new core functionalities
...
class MLDataHandler(DataHandler):
  """Collects data with ML purposes
  """
...
class PreProcessDataHandler(DataHandler):
  ...
  Collects raw data we still need to preprocess to build the input data format for ML purposes. 
  """
  def __init__(self, pollutant:str):
    self.data_handler = DataHandler(pollutant)
  ...
  def data_collector(self, date):
    ...
    Params
    ------
      :date: str = date we are willing to collect (it will contain the data for the whole month)
    ...
    -------
      ...
      :transform: tuple = containing values used for the geotransformation
      :no_data: str = missing data
      :path_to_csv: str = path to csv containing data
    """
...
class MLWorker(MLModelsConfig):
  """Class containing all needed functions to execute a ML algorithm
  """
