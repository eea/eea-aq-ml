...
#       logging.info(f'Preparing training model {ml_models_config.model_str} for pollutant {pollutant} and {type_of_params.upper()} params: {ml_params}') 
...
#           run_id = mlflow.active_run().info.run_id
#           print(f'Registering model: {model_to_train_details["model_name"]}...')
#           # The default path where the MLflow autologging function stores the model
#           artifact_path = "model"
#           model_uri = "runs:/{run_id}/{artifact_path}".format(run_id=run_id, artifact_path=artifact_path)
#           model_details = mlflow.register_model(model_uri=model_uri, name=model_to_train_details['model_name'])
...
#       else:
#         logging.info('Performing predictions with Validation features:\n ', validation_X.count())
